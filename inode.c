...
int inode_indexlookup(struct unixfilesystem *fs, struct inode *inp, int blockNum) {  
    ...
    uint16_t inode_blocks = (inode_file_size + DISKIMG_SECTOR_SIZE - 1) / DISKIMG_SECTOR_SIZE;
    ...
    if (indir_block_num < 7){
        ...
        return block;
            ...
            uint16_t double_indir_block_num = (blockNum - 7*256) / 256;
            uint16_t double_offset = (blockNum - 7*256) % 256;
            ...
            uint16_t block = second_indir_block[double_offset];
            free(indir_block);
            free(second_indir_block);
            return block;
        }
